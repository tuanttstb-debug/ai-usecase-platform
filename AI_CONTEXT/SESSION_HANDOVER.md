...
## Session: 2026-09-21 — CR Kế hoạch T9–T12 ở panel "Hàng đợi Review" + truy gốc "DB không lưu Action Plan" (data fix qua Editor)
- **Task completed:** Anh giao CR: panel chấm điểm US ở "Hàng đợi Review" phải hiển thị đầy đủ "Kế hoạch hành động theo tháng (T9–T12)" làm căn cứ cho teamlead chấm. (1) Thêm mục render vào bộ chi tiết dùng chung `uc-detail-view.js` (normalize +`action_plan_m09..m12`; render +mục 5) — review-queue + dashboard cùng dùng. (2) Anh phản hồi "không thấy ở Hàng đợi Review / hình như DB chưa lưu" → **kiểm tra LIVE bằng cách gọi route đọc GAS** (list/usecase/h1-list) từ tab cùng origin: phát hiện `MASTER_DATA` **thiếu hẳn 4 cột `Action_Plan_M09..M12`** (102 cột vật lý = 106 chuẩn − 4; còn cột kế hoạch CŨ Execution_Plan/Planned_*). h1-list CÓ action_plan → code 12/09 đã deploy ⇒ gốc = **dữ liệu/sheet**, không phải code. (3) Truy nhân–quả: BUG-1 (18/09) khôi phục header copy từ `Data H1` (chưa có 4 cột) làm rớt; create/update KHÔNG self-heal MASTER. (4) Dựng tiện ích `ensureMasterActionPlanColumns()` (append cột thiếu ở cuối) → **[TT] chạy trong Editor → verify live 101→105 cột (4 cột đã có, US cũ rỗng)**. (5) Thêm `opts.planPlaceholder` để panel chấm điểm luôn hiện mục T9–T12 (ghi "chưa có kế hoạch" khi trống); dashboard giữ auto-ẩn.
- **Files changed:** *(SPOKE — ĐÃ COMMIT + PUSH origin/main)* `1c97c62`: `assets/js/uc-detail-view.js` (normalize+render mục 5) · `assets/gas-backend/AdminService.gs` (`listUseCases_` +4 field action_plan — **deploy-gated**) · `index.html` (cache-bust) · `tests/03-review-queue.spec.js` (+test có dữ liệu). `b52bf22`: `assets/js/uc-detail-view.js` (+`planPlaceholder`) · `assets/js/review-queue.js` (truyền planPlaceholder 2 chỗ) · `assets/gas-backend/FixMasterHeaders.gs` (+`ensureMasterActionPlanColumns()`/dryRun standalone) · `index.html` (cache-bust `?v=20260921b`) · `tests/03-review-queue.spec.js` (+test placeholder). *(GAS live — [TT] chạy Editor, ngoài git)* MASTER_DATA +4 cột. *(KHÔNG stage — rác tồn)* 11 png screenshots/h2 deleted + `~$SD_...docx`.
- **Decision made:** (a) Gốc "không thấy" = **dữ liệu** (thiếu cột), code FE render đúng — không sửa nhầm hướng. (b) Fix cột qua **hàm Editor append-only** (idempotent, an toàn) thay vì `fixMasterHeaders()` cũ (chặn khi số cột lệch — chỉ sửa lệch tên). (c) Không redeploy Web App cho phần data-fix. (d) Placeholder chỉ bật ở panel chấm điểm (review-queue) để teamlead thấy mục; dashboard giữ auto-ẩn tránh rối. (e) `AdminService.listUseCases_` +4 field để render lần đầu có sẵn — deploy-gated, chờ đợt redeploy chung.
- **Blocker:** **Không.** (Việc còn lại thuộc [TT]/chủ US.)
- **Next step:** [TT] (a) đăng ký/cập nhật 1 US test → xác nhận Kế hoạch T9–T12 lưu vào DB & panel "Hàng đợi Review" hiện đầy đủ; (b) thông báo **chủ US nhập lại kế hoạch cho US cũ** (dữ liệu đã rớt trước khi có cột) qua "Cập nhật US"; (c) 🟠 redeploy GAS dồn CR (để `listUseCases_` +4 field + các CR treo có hiệu lực). [CC] khi anh cần: sau redeploy verify list route trả action_plan; smoke vòng chấm điểm thật.
- **Regression risk:** **Rất thấp.** FE thuần render + component/opts additive (mục tự ẩn khi trống, dashboard không đổi); GAS util **append-only cột rỗng** (idempotent, KHÔNG đụng dữ liệu/cột cũ, KHÔNG redeploy); `node --check` PASS; Playwright **127/127**; verify LIVE: 4 cột `Action_Plan_M*` đã có trong MASTER_DATA (101→105). Data-boundary: 0 PII/secret; chỉ đọc route công khai + [TT] tự chạy Editor.

## Session: 2026-09-18 — Đóng BUG-1 (header MASTER_DATA) + review WF/US Team Số (nhóm 4) + seed +4 US generic
- **Task completed:** **(1) Đóng BUG-1:** [TT] khôi phục hàng header sheet `MASTER_DATA` live = copy header cũ từ tab `Data H1` (đang đúng) → hết rỗng cột định danh; mở khóa Đăng ký/Cập nhật US/My Cases/Dashboard/Leaderboard. Tiện ích `FixMasterHeaders.gs` (standalone dry-run→fix→restore, guard 106 cột + dò cột định danh + backup undo) đã dựng cho việc này (anh làm tay, kết quả tương đương). **(2) Review WF/US Team Số** (nhóm "4. Workflow đặc thù Số hóa tín dụng") — **pull LIVE catalog** qua route `workflow-list` (HTTP 200/38KB): xác nhận **7 WF / 21 US, tất cả active**, khớp seed. Phỏng vấn 3 vòng chốt north-star: **giữ WF role-specific, viết US GENERIC (không neo sản phẩm) để team khác cùng loại tái dùng + đủ chín đóng Agent; KHÔNG đụng lớp common nhóm 1**. Verdict: **GIỮ cả 7 WF (không cắt)**; 21 US hiện đã product-agnostic (không sửa). Phát hiện phụ: nhóm 1 (chung) đang trùng 2 thế hệ taxonomy (WF1–7 vs WF8–12) — nợ chuẩn hóa lớn hơn, ngoài scope. **(3) Bổ sung 4 US generic** cắm WF sẵn có: WF1 requirement→User Story+AC · WF3 RTM truy vết kiểm thử · WF4 sổ rủi ro–issue · WF6 approval-readiness.
- **Files changed:** *(SPOKE — commit phiên này)* MỚI `assets/gas-backend/WorkflowSeedTeamSoAdd.gs` (seed +4 US, **standalone** idempotent: dryRun→seed, dedup `Nhom||WF||US` bất biến khoảng trắng, guard WF lệch tên) · MỚI `assets/gas-backend/FixMasterHeaders.gs` (tiện ích khôi phục header MASTER_DATA) · 4 file `AI_CONTEXT/` spoke. *(KHÔNG stage — để nguyên)* 11 png `screenshots/h2/*` báo deleted (theo handover 2026-09-15) + rác `~$SD_...docx`.
- **Decision made:** (a) **Bỏ hướng neo sản phẩm** (BLOL/GNOL/eGP) — US phải generic/tái dùng. (b) "Dùng chung" = **role-agnostic** (nhóm 1); nhóm 4 giữ role-specific → không có WF common trong nhóm 4 (đúng thiết kế). (c) **GIỮ cả 7 WF**: dưới lăng kính Agent, lo ngại "team không có SDET / không có log" ở lượt đầu KHÔNG còn đúng (Agent chính là thứ làm auto-test/process-mining). (d) Seed file phải **STANDALONE** (tự mở spreadsheet + tự có helper) vì [TT] chạy trong script rỗng "Chưa có tên" → bản đầu phụ thuộc backend báo `ReferenceError: ensureWorkflowSheets_`; đã viết lại tự chứa.
- **Blocker:** **Không.** (Việc chờ đều thuộc [TT]: chạy seed trong Editor; redeploy GAS dồn CR chưa deploy từ các phiên trước.)
- **Next step:** [TT] (a) chạy `dryRunSeedTeamSoAddUS()` (xác nhận "THÊM 4 / 0 cảnh báo WF lệch tên") → `seedTeamSoAddUS()` trong GAS Editor (không cần redeploy Web App); (b) 🟠 redeploy GAS dồn CR chưa deploy (`ExerciseService.gs` vá BUG-2 + Bài tập AI · file MỚI `SelfScoreService.gs` · mapper H1 · 3 CR đăng ký tối giản/Cập nhật US/US H1); (c) dọn 4 bản ghi `[TEST]`. [CC] sau khi [TT] seed → (tùy chọn) verify `workflow-list` +4 US; đã cross-ref hub.
- **Regression risk:** **Rất thấp.** Seed **idempotent** chỉ append, KHÔNG redeploy Web App, KHÔNG sửa WF/US cũ; guard cảnh báo WF đích lệch tên tránh WF mồ côi; `node --check` PASS (bản standalone). BUG-1 = sửa **dữ liệu sheet**, 0 đụng code. Data-boundary: 0 PII/secret; WF/US = tên quy trình nội bộ generic.

## Session: 2026-09-15 — TEST LIVE toàn bộ tính năng mới (production, user tuantt4/Admin) + truy vết 2 bug + VÁ BUG-2 (ExerciseService)
- **Task completed:** Anh đã login production; CC test live qua Chrome MCP **14 case** cho mọi tính năng luồng mới (FE `?v=20260914`, GAS đã redeploy — đăng ký không còn require Pain_Point). **PASS:** US H1 (297 US, lọc + modal chi tiết), Bài tập AI (đăng bài EX-0001 + **uiConfirm** khi xóa), Tự chấm KPI (KPI2 8/7/8/6→74/100 tự tính, Nộp token-auth persist đúng), Duyệt chấm điểm (bản nộp vào hàng chờ → **Approve** SS-0001 chốt, vòng end-to-end chạy), UI/UX (Font Awesome, badge/modal theo Component Contract). Đăng ký US CR1 PASS ở UI/API (ra AIUS-0001). **2 BUG tìm được + truy vết tận gốc** (chi tiết `docs/TEST_LIVE_20260915.md`).
- **Files changed:** *(SPOKE — commit phiên này)* **VÁ** `assets/gas-backend/ExerciseService.gs:57` (BUG-2: filter Active dùng `.toUpperCase()` + so boolean — khớp convention LookupService/AuthTokenService) · MỚI `docs/TEST_LIVE_20260915.md` (test-case log + 2 phân tích bug + hướng sửa) · 4 file `AI_CONTEXT/` spoke. *(KHÔNG commit)* working-tree có sẵn 11 png `screenshots/h2/` báo deleted + rác Word `~$SD_...docx` — để nguyên, không stage.
- **Decision made:** BUG-1 = lỗi **dữ liệu/sheet** (không sửa code — code repo đúng & đã deploy đúng bản mới); vá BUG-2 tối thiểu 1 dòng theo convention sẵn có (không refactor). Test data gắn nhãn `[TEST]` để dễ dọn. Không commit screenshot deletions/temp docx (nhiễu working tree, không thuộc phiên).
- **Blocker:** **BUG-1 (🔴, việc [TT]):** picker "Cập nhật US" trống + Đăng ký/My Cases/Dashboard/Leaderboard hỏng vì **dòng header sheet `MASTER_DATA` live không khớp tên cột code** (`action=list` trả 19 record rỗng toàn bộ cột định danh). Đối chứng: cùng `readSheetAsObjects_`, sheet `Data H1` → 297 record đầy đủ → gốc 100% ở sheet MASTER_DATA. **Không phải bug code** → [TT] sửa header trên spreadsheet `1xLMQLTgj2sRf1l9C6s6AHCT5zWJLQOofL375t8Pv_NA` (đối chiếu tab `Data H1`).
- **Next step:** [TT] (a) 🔴 **sửa header MASTER_DATA** (BUG-1) → mở khóa đăng ký/cập nhật/dashboard; (b) 🟠 **redeploy GAS** với `ExerciseService.gs` đã vá (BUG-2) → xóa Bài tập AI mới ẩn được; (c) dọn 4 bản ghi test (AIUS-0001 · EX-0001 · SS-0001 · PERSONAL_SCORE tuantt4 T09). [CC] sau khi [TT] sửa MASTER_DATA → smoke lại Đăng ký/Cập nhật US xác nhận đọc-ghi khớp; (tùy chọn) đồng bộ latent `AdminService.gs:512` (`stage_changed` thiếu `|| === true`, luồng milestone đang ngủ).
- **Regression risk:** **Thấp.** BUG-2 fix cô lập 1 filter, khớp convention toàn dự án (mọi chỗ khác đã dùng `.toUpperCase()`); mock Playwright trả chuỗi 'FALSE' → logic mới vẫn lọc đúng; `node --check` PASS. Chưa hiệu lực live tới khi [TT] redeploy. Test live **chỉ tạo dữ liệu** (`[TEST]`), KHÔNG sửa/xóa dữ liệu thật; approve self-score chỉ ghi PERSONAL_SCORE của chính tuantt4 (test). Data-boundary: 0 PII/secret; mật khẩu do trình duyệt autofill (CC không nhập/không đọc).

## Session: 2026-09-14 — Rà soát UI/UX + đồng nhất các màn CR (US H1 · Bài tập AI · Tự chấm/Duyệt KPI)
- **Task completed:** Anh giao rà soát tổng thể UI các nội dung vừa triển khai, tham khảo concept SHTD (nút/tìm kiếm/popup/font/cỡ chữ/logic hiển thị), đồng nhất trải nghiệm + chưng rule chung cho dự án tới. Đối chiếu code thật SHTD + hệ component sẵn có AIUS → **10 phát hiện** đồng nhất (báo cáo `docs/UIUX_AUDIT_20260914.md`), sửa trọn FE-only.
- **Files changed:** `index.html` (nạp Font Awesome 6; 2 ô search → `.search-box`+icon; modal US H1 → `.modal-overlay/.modal-card--wide/.modal-close`; `btn--sm`→`btn-sm`; include ui-confirm.js; cache-bust `?v=20260914`) · MỚI `assets/js/ui-confirm.js` (`uiConfirm()` Promise) · `assets/css/components.css` (+`.list-empty`/`.list-loading`/`.table-wrap`) · `assets/js/ai-exercise.js` (class nút chuẩn + FA + confirm xóa + list-empty) · `assets/js/self-score.js` (badge `.badge-*` token + nút Duyệt/Từ chối class chuẩn + FA + list-empty/loading) · `assets/js/us-h1.js` (bảng `.data-table`+`.table-wrap` + modal `.hidden` + FA + guard keydown) · `tests/13-us-h1.spec.js` (selector nút đóng → `.modal-close`). *(hub)* MỚI `AIOS/04_Knowledge/references/REF-UIUX-COMPONENT-CONTRACT.md` + INDEX + trỏ chéo REF + CHECKLIST skill design-system.
- **Decision made:** quy ước class **1 gạch** (`.btn-sm`/`.btn-ghost`) là canon; trạng thái/màu qua token (không hardcode hex); 1 component search/modal/empty/table dùng lại; thao tác xóa qua `uiConfirm()`; icon = **Font Awesome 6** (khớp SHTD, anh chốt). Giữ hook `.h1-table` cạnh `.data-table` cho test.
- **Blocker:** **Không.** Thuần FE — KHÔNG cần redeploy GAS.
- **Next step:** [TT] hard-refresh `?v=20260914` nghiệm thu: US H1 bảng + modal chuẩn + icon FA hiện; Bài tập AI nút Sửa/Xóa/Copy có icon + xóa hỏi xác nhận; Tự chấm/Duyệt badge trạng thái đúng ở cả sáng/tối. [CC] khi anh cần: lan sang chuẩn hóa các màn còn lại (dashboard/leaderboard) theo cùng contract; self-host FA nếu cần offline.
- **Regression risk:** **Thấp → verify đầy đủ.** Playwright **124/124 PASS** (full suite); node --check 4 file JS OK; thuần FE trình bày + component sẵn có, 0 chạm GAS/logic nghiệp vụ. FA qua CDN (icon decorative, không vỡ khi offline). Data-boundary: 0 PII/secret.

## Session: 2026-09-12 #3 — CR Chấm điểm cá nhân (member tự chấm → teamlead duyệt) + Lan tỏa AI + polish nhãn
- **Task completed:** Sau khi anh chốt 8 quyết định thiết kế (xem `docs/PLAN_SCORING_CR_20260912.md` §QUYẾT ĐỊNH): xây trọn CR — (1) **Member tự chấm KPI2 (năng lực AI 0–10, 4 tiêu chí) + KPI3 (số khóa thường/trả phí)** + link bằng chứng tách KPI2/KPI3 → nộp; (2) **Lan tỏa AI (KPI4)** member tự khai + bằng chứng → nộp; (3) **Teamlead/admin duyệt (approve/reject, reject bắt buộc lý do)** — Approve chốt sang PERSONAL_SCORE / set Sharing_Achieved. Mô hình **Phương án B**: staging sheet, KHÔNG đụng engine KPI (`_memberKpiFor_`), điểm chỉ tính sau Approved. Giữ màn teamlead chấm trực tiếp cũ song song; lan tỏa bổ sung song song auto UC_REUSE≥3; 1 lần/tháng dương lịch (sửa trước duyệt, khóa sau Approved). Kèm **polish** nhãn "Cập nhật tuần"→"Cập nhật US" (menu/home/title/helper) + card home US H1/Bài tập AI. Đóng gói **kết quả test live** vào PROJECT_STATE.
- **Files changed:** *(GAS — redeploy, có FILE MỚI)* MỚI `assets/gas-backend/SelfScoreService.gs` · `Config.gs` (+SHEETS.PERSONAL_SELF/SHARING_CLAIM + 2 headers) · `Code.gs` (+7 route self-score-*/sharing-claim-*). *(FE)* MỚI `assets/js/self-score.js` · `assets/js/api.js` (+7 method) · `config/routes.js` (+7 route) · `assets/js/shell.js` (card home: Cập nhật US + US H1 + Bài tập AI) · `assets/js/weekly-update.js` (router title) · `assets/js/constants.js` (helper text) · `index.html` (+nav "Tự chấm KPI"/"Duyệt chấm điểm" + 2 section + include + cache-bust `?v=20260912c`). *(test)* MỚI `tests/15-self-score.spec.js` (4). *(doc)* `docs/PLAN_SCORING_CR_20260912.md` (+§QUYẾT ĐỊNH đã chốt).
- **Decision made:** review=approve/reject; KPI2 member tự chấm; KPI3 giữ công thức 4 khóa=100%(trả phí×2), member khai thường+trả phí; kỳ tháng dương lịch, khóa sau Approved; lan tỏa bổ sung song song (đạt nếu Sharing_Achieved HOẶC reuse≥3); giữ teamlead chấm trực tiếp; bằng chứng tách KPI2/KPI3/KPI4; toàn bộ member; **Phương án B (staging, không đụng engine)** để giảm rủi ro.
- **Blocker:** **[TT] REDEPLOY GAS** — nhớ **THÊM FILE MỚI `SelfScoreService.gs`** (dễ quên → route sẽ ReferenceError như CR4/exercise). 2 sheet `PERSONAL_SELF`/`SHARING_CLAIM` tự tạo khi dùng lần đầu (ensureSheetColumns_). Ngoài ra CR2 live cần [TT] kiểm sheet MASTER_DATA + dán lại trọn AdminService.gs (xem PROJECT_STATE).
- **Next step:** [TT] redeploy GAS (đủ file mới) → hard-refresh `?v=20260912c` → nghiệm thu: member vào "Tự chấm KPI" nhập→nộp; teamlead vào "Duyệt chấm điểm" approve/reject; sau approve KPI cập nhật. [CC] sau redeploy: smoke 1 vòng member-nộp→teamlead-duyệt thật (kiểm PERSONAL_SELF + PERSONAL_SCORE + Sharing_Achieved).
- **Regression risk:** **Thấp→verify.** Phương án B KHÔNG sửa engine KPI/submitPersonalScore_ cũ (chỉ thêm ghi vào PERSONAL_SCORE khi approve, cùng khuôn upsert). Playwright 124/124 (mock). GAS syntax OK. Auth: chính chủ nộp / teamlead-của-team(+backup)/admin duyệt. Data-boundary: chỉ link bằng chứng (không upload), 0 PII/secret.

## Session: 2026-09-12 #2 — Tính năng "Bài tập AI" + nâng cấp US H1 (xem chi tiết + lọc theo trường chung)
- **Task completed:** (1) **Bài tập AI** — loại nội dung nhẹ chia sẻ thao tác AI nhỏ (chưa thành US), không tính KPI: sheet mới `AI_EXERCISE`, `ExerciseService.gs` (list/create/update/delete — xóa mềm; sửa/xóa chủ bài hoặc admin), routes `exercise-*`, module FE `ai-exercise.js` + menu "Bài tập AI" (1 trang: form đăng Tiêu đề*/Mô tả/Prompt*/Link demo ổ chung + tra cứu tìm kiếm + xem prompt/copy inline + Sửa/Xóa của mình + hint "tuần này đã đăng X bài", không chặn). (2) **US H1 nâng cấp:** `listUseCasesH1_` trả thêm field nội dung (pain/process/flow/prompt_*/when/usage/business_category); `us-h1.js` thêm modal **xem chi tiết** (bấm dòng) + **lọc Team/Workflow/Stage** + mở rộng tìm kiếm.
- **Files changed:** *(GAS — redeploy)* MỚI `assets/gas-backend/ExerciseService.gs` · `Config.gs` (+SHEETS.AI_EXERCISE + AI_EXERCISE_HEADERS) · `Code.gs` (+4 route exercise-*) · `AdminService.gs` (mapper `listUseCasesH1_` +field chi tiết). *(FE)* MỚI `assets/js/ai-exercise.js` · viết lại `assets/js/us-h1.js` (detail modal + filters + search rộng) · `assets/js/api.js` (+list/create/update/deleteExercise) · `config/routes.js` (+exercise* routes) · `index.html` (nav+section "Bài tập AI"; view us-h1 +filter bar +modal chi tiết; cache-bust `?v=20260912b`). *(test)* `tests/13-us-h1.spec.js` (+T04 detail +T05 filter, mock enrich) · MỚI `tests/14-ai-exercise.spec.js` (4 test).
- **Decision made:** Bài tập AI tách hẳn US/KPI; nhịp 1 bài/tuần chỉ khuyến khích (không chặn); demo = link ổ chung (không upload → data-boundary); auth sửa/xóa theo mô hình nhẹ (client gửi requester_email+is_admin, server so Owner_Email hoặc admin) giống weekly-update — xem TECH_DEBT. US H1 detail đọc từ chính h1-list (mở rộng mapper, không thêm round-trip); modal chỉ đọc.
- **Blocker:** **[TT] REDEPLOY GAS** (thêm ExerciseService + routes + mapper H1). Sheet `AI_EXERCISE` tự tạo khi đăng bài đầu.
- **Next step:** [TT] redeploy GAS → hard-refresh `?v=20260912b` nghiệm thu: đăng/tra cứu Bài tập AI; US H1 bấm dòng xem chi tiết + lọc Team/Workflow/Stage. [CC] (tùy chọn) sau redeploy smoke: đăng 1 Bài tập AI thật (kiểm sheet AI_EXERCISE sinh EX-0001).
- **Regression risk:** **Thấp → verify.** Playwright **120/120**; GAS syntax OK; tính năng mới cô lập (sheet/route/module riêng); US H1 chỉ mở rộng mapper + FE view (read-only). Data-boundary: demo link ổ chung, 0 upload; 0 PII/secret.

## Session: 2026-09-12 — 3 CR: Đăng ký US tối giản + "Cập nhật US" (thay "Cập nhật tuần") + View "US H1" chỉ đọc
- **Task completed:** Triển khai trọn 3 CR liên đới (FE + GAS), staged theo plan `docs/PLAN_CR_20260912.md`. **CR1:** đăng ký còn 1 bước (Workflow→Use Case + 4 ô Action Plan T9–T12, bắt buộc ≥1 tháng); bỏ painpoint/quy trình/đối tượng/mục tiêu + prompt/demo/hướng dẫn + Stage khỏi đăng ký; +4 cột `Action_Plan_M09..M12`; sửa `REQUIRED_FIELDS_CREATE`; US mới mặc định Stage S1 (GAS). **CR2:** màn "Cập nhật tuần"→"Cập nhật US" (editor): Prompt/Luồng AI mở sẵn = nội dung chính + Demo + 4 ô Action Plan + số liệu; ghi chú không bắt buộc; bỏ nâng Stage/checklist (stageSection giữ ẩn). **CR3:** menu "US H1" + route `h1-list` đọc sheet `Data H1` (read-only, tách khỏi H2).
- **Files changed:** *(FE)* `assets/js/constants.js` (STEPS 1 bước + 4 FIELDS/FIELD_CONFIG action plan + GROUP plan) · `assets/js/validation.js` (bỏ required pain/process/flow, thêm action-plan ≥1) · `assets/js/wizard.js` (single-step chrome, bỏ step2) · `assets/js/weekly-update.js` (bỏ render/nâng Stage, prefill Action Plan+Demo, prompt mở sẵn, submit gửi Action Plan+Demo) · MỚI `assets/js/us-h1.js` · `assets/js/api.js` (+listUseCasesH1) · `config/routes.js` (+h1List) · `index.html` (register 1 bước + section weekly-update reframe + Action Plan/Demo + nav "Cập nhật US"/"US H1" + section us-h1 + include + cache-bust `?v=20260912a`). *(GAS — cần redeploy)* `Config.gs` (+4 cột HEADERS, +SHEETS.MASTER_H1, sửa REQUIRED_FIELDS_CREATE) · `UseCaseService.gs` (default Stage S1) · `AdminService.gs` (submitWeeklyUpdate_ ghi Action Plan+Demo; +listUseCasesH1_) · `Code.gs` (+route h1-list). *(test)* viết lại `tests/weekly-update.spec.js` (mock GAS, 5 test CR2) · MỚI `tests/13-us-h1.spec.js` (3 test) · `tests/05-create-write-ops.spec.js` (MIN_VALID + Action Plan, D2/D3 payload lớn qua Action Plan). MỚI `docs/PLAN_CR_20260912.md`.
- **Decision made:** đăng ký tối giản triệt để (nội dung dời sang "Cập nhật US"); Action Plan = 4 ô tháng, bắt buộc ≥1; Stage retire CHỈ ở đăng ký+cập nhật (giữ cột+pill dashboard/thư viện, backward-compat); "Cập nhật US" thay hẳn "Cập nhật tuần" (bỏ báo cáo tuần bắt buộc + nâng Stage, GIỮ timeline "Lịch sử cập nhật"); US H1 = menu riêng chỉ đọc, sheet `Data H1` cột giống MASTER_DATA. Giữ `data-view="weekly-update"` (chỉ đổi nhãn) để không xáo router. stageSection giữ DOM ẩn (tránh sửa khối SVG lớn) → dead-DOM ghi TECH_DEBT.
- **Blocker:** **[TT] REDEPLOY GAS** trước khi dùng — CR1 sẽ LỖI trên GAS cũ (server còn require Pain_Point/Current_Process). Không có blocker kỹ thuật khác.
- **Next step:** [TT] (a) **redeploy GAS** (Config+UseCaseService+AdminService+Code); (b) đảm bảo tab **`Data H1`** tồn tại (cột giống MASTER_DATA) — thiếu thì view US H1 hiện "Chưa có dữ liệu" (không lỗi); (c) hard-refresh `?v=20260912a` nghiệm thu: đăng ký 1 bước ghi được US + Action Plan; "Cập nhật US" sửa prompt/demo/action plan; menu "US H1" liệt kê US kỳ H1. [CC] sau khi anh redeploy: (tùy chọn) smoke tạo 1 US thật + kiểm 4 cột action plan xuất hiện trong MASTER_DATA.
- **Regression risk:** **Trung bình → đã verify.** Chạm register/weekly-update/nav + GAS write path. Giảm rủi ro: Playwright 114/114 (mock, deterministic); GAS syntax OK ×4; cột mới append cuối HEADERS (ensureSheetColumns_ self-heal, không lệch cột cũ); Stage giữ backward-compat; h1-list read-only + guard sheet vắng→[]. Data-boundary: 0 PII/secret.

## Session: 2026-09-11 — Seed WORKFLOW_CATALOG PTKD & QLDM (+33 US / 7 workflow) từ Excel [gốc=spoke-data, quản trị tại hub AIOS]
- **Task completed:** Nạp US/WF mới của **PTKD & QLDM** vào catalog droplist đăng ký UC. Nguồn = Excel (hub `binh-dan-hoa-ai-H2/data/Workflow và Use case PTKD + QLDM.xlsx`, sheet "PO - PTKD"). Trích 77 dòng → ánh xạ Nhóm canonical khớp `Config.gs TEAM_GROUP_SEED`: PO→`2. Workflow đặc thù PO` · chung→`1. Workflow chung` · PTKD→`3. Workflow PTKD & QLDM` (**QLDM chia sẻ nhóm 3 với PTKD MB/MN**). Pre-check read-only GET `workflow-list` (109 dòng live) → thêm **33 US** (PO +0 · chung +2 · PTKD&QLDM +31 / 7 workflow); TEAM_GROUP_MAP PTKD MB/MN/QLDM đã có sẵn live.
- **Files changed:** MỚI `assets/gas-backend/WorkflowSeedDataPTKD.gs` (data nhúng + `seedWorkflowCatalogPTKD()` idempotent + `dryRunSeedWorkflowCatalogPTKD()`; khuôn giống `WorkflowSeedDataPO.gs`/`TeamSo.gs`). Vá `_ptkdWfKey_` gộp `\s+`→1 space (dryRun editor 34→33, khử 1 gần-trùng PO dấu-cách-đôi). `node --check` PASS.
- **Decision made:** nguồn sheet "PO - PTKD"; QLDM dùng chung nhóm 3; cơ chế = file GAS seed chạy Editor (idempotent, tránh auto-classifier); dedup bất biến khoảng trắng. (Hub `binh-dan-hoa-ai-H2/DECISIONS.md` D30.)
- **Blocker:** Không (chờ [TT] chạy seed trong Editor).
- **Next step:** [TT] dán lại `.gs` đã vá → `dryRunSeedWorkflowCatalogPTKD()` xác nhận **33** (PO=0) → `seedWorkflowCatalogPTKD()` → hard-refresh: droplist PTKD MB/MN + QLDM thấy 7 workflow mới. **KHÔNG cần redeploy Web App** (chỉ ghi sheet). [CC] verify `workflow-list` +33 sau khi seed.
- **Regression risk:** **Rất thấp** — seed idempotent, chỉ append, không sửa dòng cũ, không redeploy. Verify: pre-check local 33 khớp dryRun 33; PO +0 chứng minh không đụng taxonomy cũ. Data-boundary: US/WF = tên quy trình nội bộ, 0 PII/secret (cột phân công người trong Excel đã bỏ).

## Session: 2026-09-09 #2 — Tái kiến trúc SPA-lite (12 trang → 1 shell) + gỡ duyệt US (nhánh feat/spa-shell)
- **Task completed:** Anh báo bug "chuyển tab left-menu hiện thêm/thiếu button" → chẩn đoán gốc = **multi-page nhân bản khung → sidebar drift** (bằng chứng: đếm nav lệch giữa 12 trang). Anh chốt **Full SPA (như SHTD)**. Làm trọn theo plan (staged, commit từng bước, verify từng view): **(1)** Shell `index.html` 1 nguồn (sidebar hash-nav + RBAC `data-roles` + topbar chuẩn: dark+user-pill+logout) + `router.js` (hash router) + `shell.js`. **(2)** Gộp **10 view** vào `<section data-view>` + đổi module sang `Router.register` (bỏ tự-boot); tách 2 inline lớn (leaderboard/weekly) → module IIFE + CSS riêng. **(3)** **Gỡ duyệt US** (FE+api+routes+GAS), giữ milestone. **(4)** Cache-bust `?v=` + 12 trang cũ→stub. **(5)** Vá test SPA (nav-id/URL/expose global) → **118/118**.
- **Files changed:** *(nhánh feat/spa-shell — commit S0..S3)* MỚI `assets/js/{router.js,shell.js}` · MỚI `assets/css/{leaderboard.css,weekly-update.css}` (tách inline) · MỚI `assets/js/{leaderboard.js,weekly-update.js}` (tách inline) · MỚI `docs/DESIGN_Topbar_Button_Layout.md` · viết lại `index.html` (shell) · sửa 8 module view (library/workflow-coverage/workflow-catalog/personal-score/review-queue/app+wizard/dashboard) bỏ boot→register · `dashboard.js`+`api.js`+`config/routes.js`+`AdminService.gs`+`Code.gs` (gỡ duyệt US) · 9 trang cũ→stub · `tests/*` vá SPA · gỡ `assets/js/topbar.js` (thừa). + 4 `AI_CONTEXT/`.
- **Decision made:** SPA-lite giữ multi-file routing bằng hash (không framework); mỗi view lazy-init 1 lần qua router registry. `register`+`dashboard` dùng `show(sub)` (form/tab tươi mỗi entry; sub=edit-id/tab). `change-password`+`login` giữ standalone (pre-auth/layout riêng). Gỡ duyệt US = **chỉ US** (anh chốt), milestone giữ vì gắn KPI tuần. Nút detailApprove/Reject GIỮ (milestone dùng chung), chỉ bỏ nhánh US. Concept topbar lưu doc trước khi chuẩn hóa (anh yêu cầu).
- **Blocker:** (a) đang chạy full suite lần cuối xác nhận 0 regression → rồi **merge main + push**; (b) **⚠️ [TT] REDEPLOY GAS** (AdminService.gs+Code.gs gỡ route approve/reject US) — milestone không đổi.
- **Next step:** [CC] full xanh → cập nhật cross-ref hub + merge `feat/spa-shell`→main + push. [TT] redeploy GAS + hard-refresh nghiệm thu: chuyển tab **menu KHÔNG đổi số mục/nhấp nháy** (hết drift); đăng ký US → vào review luôn (không nút duyệt); logout topbar; Ctrl+D dark. [CC] (tùy chọn) TD-DS-03 re-theme Chart.js dark.
- **Regression risk:** **Trung bình → verify kỹ.** Chạm mọi module (bỏ boot→register) + gỡ duyệt US + viết lại điều hướng test. Giảm rủi ro: staged 8 commit, mỗi view chụp ảnh 0 lỗi, Playwright 118/118, GAS syntax OK, 0 dangling ref. Trang cũ .html thành stub (module không self-boot nữa nên .html gốc "chết" — đã thay bằng stub redirect). Data-boundary: 0 secret/PII.

## Session: 2026-09-09 — Chuẩn hóa thiết kế Pha 2: nút primary→CAM + DARK MODE toggle (12 trang)
- **Task completed:** Đóng pha 2 (2 nợ TD-DS-01/02) theo REF-UIUX-DESIGN-SYSTEM. **(1) Nút primary tím→cam** (`.btn-primary`→`--color-accent`, hover accent-dark, glow cam) — tím còn cho cấu trúc; đúng Nguyên tắc 2 "1 sắc nhấn cho hành động". **(2) Dark mode toggle:** MỚI `assets/js/theme.js` tự chứa (apply sớm chống FOUC · nhớ `localStorage aius_theme` · chèn nút vào `.topbar-actions` SVG trăng/mặt trời, fallback nút nổi cho login/đổi-mật-khẩu · phím Ctrl+D) + thêm `<script>` vào `<head>` **12 trang**. **(3) Vá token dark:** `color-scheme`, `--overlay-loading` đảo, `*-light`→tint trong suốt, `--color-border-focus` sửa lệch màu; **dark-polish** sáng chữ badge/alert/rq-badge/kpi-badge; vá 3 nền trắng literal→token (score-component, rq-filter-bar, input lỗi).
- **Files changed:** *(sửa)* `assets/css/variables.css` (color-scheme + overlay-loading + border-focus + dark block *-light/color-scheme) · `assets/css/components.css` (btn-primary→accent + floating toggle CSS + dark-polish badge/alert) · `assets/css/layout.css` (loading-overlay→token; floating CSS đã dời sang components) · `assets/css/dashboard.css` (2 nền trắng→surface + dark-polish rq/kpi badge) · `assets/css/forms.css` (input lỗi→error-light). *(MỚI)* `assets/js/theme.js`. *(12 HTML app +1 dòng `<head>`)* index/dashboard/register/login/change-password/leaderboard/workflow-catalog/workflow-coverage/review-queue/personal-score/weekly-update/library. *(test)* KHÔNG đổi. + 4 `AI_CONTEXT/`.
- **Decision made:** Theo REF: nút chính = **accent cam** (không giữ tím). `*-light` giữ **tint trong suốt** ở dark (đẹp cho ô icon KPI/stat) + override riêng chữ badge/alert cho đủ tương phản (thay vì đảo *-light về pastel). `theme.js` đặt trong `<head>` (không defer) để apply data-theme trước paint. Nút nổi (components.css, nạp mọi trang) cho 2 trang không có topbar/không nạp layout.css. Mặc định **light** trừ khi user từng đổi (không auto theo `prefers-color-scheme` để giữ đúng "gu" [TT] đã duyệt).
- **Blocker:** **Không.** Thuần FE/CSS+JS — KHÔNG cần redeploy GAS.
- **Next step:** [TT] hard-refresh production → nghiệm thu: nút "Đăng ký mới"/"Tiếp theo" nay **cam**; bấm nút góc phải topbar (hoặc **Ctrl+D**) đổi sáng/tối, F5 vẫn nhớ; login/đổi-mật-khẩu có nút nổi góc phải. [CC] (TD-DS-03, tùy chọn) re-theme màu Chart.js theo dark (nhãn/lưới) · rà status-chip trang phụ.
- **Regression risk:** **Thấp** — thuần trình bày, 0 chạm DOM/class cũ/logic/GAS/route. **Playwright 118/118 pass** (test không đổi) + chụp ảnh dark/light 5 màn xác nhận thị giác. CSS ngoặc cân bằng 5/5, `theme.js` node --check OK. Data-boundary: 0 secret/PII; localStorage chỉ lưu 'light'/'dark'.
