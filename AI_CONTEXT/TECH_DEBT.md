...
> **Delta 2026-09-21 (CR Kế hoạch T9–T12 + fix DB không lưu Action Plan):** ✅ **Đóng data-bug "MASTER_DATA thiếu 4 cột `Action_Plan_M09..M12`"** — [TT] chạy `ensureMasterActionPlanColumns()` (mới trong `FixMasterHeaders.gs`) → 101→105 cột. 🆕 **TD-MASTER-01 (MEDIUM, gốc lớp sâu):** `createUseCase_`/`updateUseCase_`/`submitWeeklyUpdate_` **KHÔNG gọi `ensureSheetColumns_(SHEETS.MASTER, HEADERS)`** → khi header MASTER bị khôi phục/lệch (như BUG-1 copy từ `Data H1` thiếu cột mới), cột mới KHÔNG tự quay lại và **ghi dữ liệu bị rớt âm thầm** (create vẫn báo thành công). Chỉ `WorkflowService.gs:263` mới self-heal MASTER. → **Nên thêm self-heal MASTER vào đầu create/update** (1 lần đọc header, rẻ; ngăn tái diễn class lỗi mất-dữ-liệu-âm-thầm) — deploy-gated, gộp vào đợt redeploy dồn CR. 🆕 **Ghi nhận:** `AdminService.listUseCases_` đã +4 field `action_plan_*` (deploy-gated) — render lần ĐẦU panel review dùng list data; trước khi redeploy, mục T9–T12 chỉ hiện sau khi enrichment `getUseCase` xong (vẫn OK vì detail đọc raw row). 🆕 **Ghi nhận (không phải nợ):** `uc-detail-view.js` dùng chung → mục T9–T12 hiện cả ở dashboard "chi tiết US" (auto-ẩn khi trống); panel review bật `planPlaceholder` để luôn hiện. Data-boundary: 0 PII/secret.

---

> **Delta 2026-09-18 (Đóng BUG-1 + review WF Team Số):** ✅ **BUG-1 ĐÓNG** — [TT] khôi phục hàng header sheet `MASTER_DATA` live (copy header cũ từ tab `Data H1` đang đúng) → hết rỗng cột định danh; Đăng ký/Cập nhật US/My Cases/Dashboard/Leaderboard hoạt động lại. Là lỗi **dữ liệu/sheet**, không đụng code. Tiện ích dựng cho việc này: `assets/gas-backend/FixMasterHeaders.gs` (standalone, dry-run→fix→restore, guard số cột 106 + dò cột định danh + backup undo) — **giữ lại** làm công cụ bảo trì khi header lệch lại. **Review WF/US Team Số: KHÔNG nợ code mới** — MỚI `assets/gas-backend/WorkflowSeedTeamSoAdd.gs` (standalone, idempotent, chỉ append; guard WF lệch tên). Ghi nhận (không phải nợ): (1) 2 file GAS này (`FixMasterHeaders`/`WorkflowSeedTeamSoAdd`) là **tiện ích chạy-tay trong Editor**, KHÔNG thuộc Web App bundle → dùng pattern **standalone tự chứa** (tự openById + tự helper) để chạy được trong script rỗng, KHÔNG dùng chung helper backend (`ensureWorkflowSheets_`…). (2) Phát hiện **nợ chuẩn hóa nhóm 1 (chung)**: đang trùng 2 thế hệ taxonomy (WF1–7 vs WF8–12 lặp domain) → cần dedup trước khi đóng Agent lớp common (ngoài scope phiên này). Data-boundary: 0 PII/secret; WF/US = tên quy trình nội bộ generic.
>
> **Delta 2026-09-15 (Test live + 2 bug):** ✅ **VÁ BUG-2** `ExerciseService.gs:57` — xóa Bài tập AI (soft-delete Active=FALSE) không hiệu lực vì **Google Sheets tự ép chuỗi 'FALSE'→boolean**, đọc lại `String(false)`='false' ≠ 'FALSE' → filter không loại. Đổi filter về `r.Active !== false && String(r.Active).toUpperCase() !== 'FALSE'` (khớp convention LookupService:56/AuthTokenService:212). **Deploy-gated: [TT] redeploy GAS.** Quét toàn backend: đây là điểm sót DUY NHẤT. 🆕 **TD-EX-03 (LOW, latent):** `AdminService.gs:512` `stage_changed: row.Stage_Changed === 'TRUE'` thiếu `|| === true` (khác dòng 524/529/576/626) → dính cùng bug ép-boolean nếu Stage_Changed lưu boolean; luồng milestone đang "ngủ" (TD-CR-02) nên chưa lộ — đồng bộ khi động vào milestone. 🆕 **Minor UI:** sau Nộp tự chấm KPI, màn my-score hiện form rỗng + badge "Nháp" (dữ liệu đã persist đúng) → nên echo trạng thái "Chờ duyệt"/khóa. **⚠️ Ngoài phạm vi code — BUG-1 (DỮ LIỆU, việc [TT]):** header sheet `MASTER_DATA` live không khớp `Config.gs` HEADERS → mọi màn đọc theo tên cột rỗng (đối chứng: `Data H1` cùng helper trả 297 record đủ). Sửa header trên spreadsheet, KHÔNG đụng code. Data-boundary: 0 PII/secret.

---

> **Delta 2026-09-14 (Rà soát UI/UX + đồng nhất):** ✅ **ĐÓNG các lỗi đồng nhất** ở màn CR (class nút sai `btn--sm`/`btn--ghost` → canon; search về `.search-box`; modal US H1 chuẩn; badge token; empty/loading/table chung; confirm xóa; FA). Playwright 124/124. 🆕 **Nợ (LOW):** (1) **TD-UI-01** — `.btn--ghost` trong `dashboard.css` nay **dead** (đã quy usage về `.btn-ghost`); xóa khi rảnh + rà nốt các nút cũ trong dashboard/leaderboard chưa theo contract. (2) **TD-UI-02** — Font Awesome nạp qua **CDN cdnjs không SRI** (tránh hash lệch chặn tải); self-host hoặc thêm SRI hash đúng nếu cần offline/an toàn CDN. (3) **TD-UI-03** — vài glyph inline nhỏ trong text (⚠/↗ ở self-score) chưa chuyển FA (không phải nút, chấp nhận). (4) `_stagePill` (us-h1) còn dựng inline theo `STAGE_LABELS` nguồn chung — chấp nhận. Nguồn chuẩn contract: `AIOS/04_Knowledge/references/REF-UIUX-COMPONENT-CONTRACT.md`. Data-boundary: 0 PII/secret.

> **Delta 2026-09-12 #2 (Bài tập AI + US H1 chi tiết/lọc):** 🆕 **Nợ (LOW):** (1) **TD-EX-01 auth nhẹ** — sửa/xóa Bài tập AI xác thực theo mô hình nhẹ: client gửi `requester_email` + `is_admin`, server so `Owner_Email`/admin (KHÔNG verify token HMAC như council). Đủ cho tính năng nội bộ low-stakes nhưng có thể giả mạo email → cân nhắc chuyển sang verify token nếu siết bảo mật. (2) **TD-EX-02** — `Exercise_ID` sinh bằng đọc max hiện có +1 KHÔNG trong lock → hiếm khi 2 người đăng đồng thời có thể trùng ID (volume thấp, rủi ro nhỏ; nâng lên counter CONFIG + lock nếu cần). (3) **TD-H1-01** — `listUseCasesH1_` giờ trả cả field nội dung (prompt/pain/flow…) trong payload list → payload nặng hơn; H1 là kho hữu hạn nên chấp nhận, tách route `h1-detail` nếu số bản ghi lớn.

> **Delta 2026-09-12 (3 CR: đăng ký tối giản + "Cập nhật US" + view US H1):** 🆕 **Nợ (LOW):** (1) **TD-CR-01 dead-DOM stageSection** — CR2 bỏ luồng nâng Stage nhưng GIỮ khối HTML `#stageSection` (upgradeToggle/checklist/s4Fields) trong `index.html`, chỉ không render/hiện (weekly-update.js không gọi `renderStageSection`, không bind `upgradeToggle`). Lý do: tránh sửa khối SVG lớn. Hàm chết còn lại trong weekly-update.js: `renderStageSection/onUpgradeToggle/onChecklistChange/getTargetStage/validateChecklist` (không caller). → xóa DOM + hàm khi rảnh. (2) **TD-CR-02** — GAS milestone infra (`New_Stage`/`submitWeeklyUpdate_` nhánh milestone + queue duyệt milestone ở dashboard) còn nguyên nhưng FE KHÔNG còn tạo milestone mới (không gửi `New_Stage`) → queue duyệt milestone thực tế "ngủ" (chỉ xử lý milestone tồn dư). Giữ để backward-compat; cân nhắc gỡ hẳn nếu chốt bỏ mô hình Stage toàn app. (3) **TD-CR-03 deploy-gated** — CR1 đổi `REQUIRED_FIELDS_CREATE` (bỏ Pain_Point/Current_Process) ở GAS → **bắt buộc [TT] redeploy GAS**, nếu không form đăng ký mới sẽ bị server chặn (thiếu trường cũ). (4) sheet `Data H1` là nguồn ngoài code — cột phải giống MASTER_DATA; đổi schema H1 cần cập nhật `listUseCasesH1_` mapper.

> **Delta 2026-09-09 #2 (Tái kiến trúc SPA-lite + gỡ duyệt US — nhánh feat/spa-shell):** ✅ **ĐÓNG gốc "drift menu khi chuyển tab"** — nguyên nhân multi-page nhân bản sidebar 12 file → gộp về **1 shell** (router hash + `<section data-view>`), sidebar/topbar/RBAC 1 nguồn. ✅ **Gỡ duyệt US** (giữ milestone). Playwright **118/118**. 🆕 **Nợ/ghi nhận (LOW):** (1) **TD-SPA-01** — dead code tự-guard còn lại sau gỡ duyệt US: `renderRejectedCard`/`updatePendingBadge`/`_rejectedList`/`_pendingList` (không còn caller US, tự return null — vô hại, xóa hẳn khi rảnh). (2) **TD-SPA-02** — `register`/`dashboard` re-init mỗi lần vào view qua `show()`; nếu click nav khi ĐANG ở đúng view thì hashchange không fire (không re-render — đúng ý), nhưng edit-mode register lần 2 khác id trong cùng phiên chưa test kỹ (luồng hiếm; edit thường vào từ dashboard = điều hướng mới). (3) **TD-SPA-03** — trang `.html` cũ nay là **stub redirect**; nếu ai mở trực tiếp file .html qua `file://` (không qua server) thì redirect vẫn chạy nhưng SPA cần server tĩnh. (4) weekly-update.js mirror 3 biến `_myUseCases/_fullDetailLoaded/_promptTouched` lên `window` chỉ để test đọc (observability, không đổi logic). **⚠️ Deploy-gated:** [TT] REDEPLOY GAS (gỡ route approve/reject US). Nguồn concept: `docs/DESIGN_Topbar_Button_Layout.md` + `AIOS REF-UIUX-DESIGN-SYSTEM`.

> **Delta 2026-09-09 (Chuẩn hóa thiết kế Pha 2 — nút primary→cam + dark mode):** ✅ **ĐÓNG TD-DS-01** (nút `.btn-primary` tím→**cam** `--color-accent`, đúng REF Nguyên tắc 2) + ✅ **ĐÓNG TD-DS-02** (dark mode: MỚI `assets/js/theme.js` tự chứa + toggle topbar/nút nổi + persist `aius_theme` + apply sớm chống FOUC + Ctrl+D; thêm `<script>` `<head>` 12 trang; vá token dark: color-scheme/overlay-loading/*-light-tint/border-focus + dark-polish badge/alert + 3 nền trắng literal→token). Thuần FE/CSS+JS trình bày, KHÔNG deploy-gated (hiệu lực ngay hard-refresh). **Playwright 118/118** (test không đổi). 🆕 **TD-DS-03 (mở, LOW):** biểu đồ Chart.js chưa re-theme theo dark (nhãn/lưới màu tối vẽ trên canvas nền tối — do JS vẽ, cần đọc option `scales`/`plugins.legend.labels.color` theo `data-theme`, tách phiên); một số status-chip sâu ở trang phụ (personal-score/review-queue) có thể còn tông chưa tối ưu (chưa gặp lỗi đọc). Nguồn chuẩn: `AIOS/04_Knowledge/references/REF-UIUX-DESIGN-SYSTEM.md`. Data-boundary: 0 secret/PII; localStorage chỉ 'light'/'dark'.

> **Delta 2026-09-08 (Chuẩn hóa thiết kế theo hệ SHTD — Pha 1):** Không nợ CRITICAL/HIGH mới. Thuần CSS (remap `variables.css` + @import font `base.css` + active bar `layout.css`), giữ nguyên kiến trúc/class/logic; Playwright 118/118 pass. **Nợ nhẹ/tồn (pha 2, tùy chọn, LOW):** ⚪ **TD-DS-01** — nút primary vẫn **tím** (`btn-primary{background:var(--color-primary)}`); hệ SHTD dùng **cam cho hành động chính** → chưa đổi để tránh thay đổi thị giác lớn (đổi = `--color-primary`→`--color-accent` ở `.btn-primary`, cân nhắc với [TT]). ⚪ **TD-DS-02** — **dark-mode CHƯA bật**: token `[data-theme="dark"]` đã có trong `variables.css` nhưng thiếu nút toggle + persist + gắn `data-theme` cho 12 trang HTML (multi-page); làm khi [TT] cần. Nguồn chuẩn: `AIOS/04_Knowledge/references/REF-UIUX-DESIGN-SYSTEM.md` + skill `design-system`. Data-boundary: 0 secret/PII.
...
> - ✅ **H1-CLEANUP-01 deploy-gated ĐÓNG** — GAS live nay đã áp gỡ routes governance + ScoringEngine + call site (không còn route H1 vô hại trên live). Còn cosmetic: CSS `.sptd-*` **đã gỡ**; cột `Auto_Score`/`Manual_Score` giữ (tương thích dữ liệu cũ, không ghi mới — chủ đích, không phải nợ).
> - ✅ **SCORING-H2-MONTHLY-01 (migration) ĐÓNG** — `setupScoringH2Sheets()` đã chạy sau deploy (cột `Month`+`Evidence_Link` có trong `PERSONAL_SCORE`). **Còn treo (cần [TT] quyết, KHÔNG đóng được bằng deploy):** nguồn NHẬP `Evidence_Link` (member nhập ở đâu / link cố định theo team) — hiện chỉ hiển thị đọc-only.
> - **Còn mở (không thuộc phạm vi clear lần này):** **WRITE-TRANSPORT-01** residual (cần browser + tài khoản test để soi iframe-POST payload >7500) · **Round 2 T2** (idempotency `reqId` server + retry write — **feature**, tách phiên riêng vì chạm transport hybrid) · **SCORING-H2-AUTH-01** (fallback reviewer_email, rủi ro thấp).

## ✅ UC-DETAIL-DUP-01 — ĐÓNG 2026-08-31 (dashboard.js nay dùng chung uc-detail-view.js)
CR review-panel (Mục tiêu 2) tách bộ render chi tiết UC ra **module dùng chung `assets/js/uc-detail-view.js`** (`UCDetailView.render/normalize/hasPrompt/copyPrompt`) cho `review-queue`. Trước 2026-08-31 `dashboard.js` vẫn giữ bản nhân bản → sửa layout chi tiết phải sửa 2 chỗ. **Đã đóng:** `_renderDetailBody` sections 1–4 → `UCDetailView.render(uc, {noEmptyFallback:true})`; dashboard nối tiếp mục ✓ phê duyệt + ★ Điểm US (gắn action) + milestone block; gỡ `_dsubsec`/`_demoField` nhân bản. `dashboard.html` +include `uc-detail-view.js`. Verify Playwright 107/107.
