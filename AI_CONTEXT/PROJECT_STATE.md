...
> ⚠️ **CHƯA smoke test login thật (username+password token path)** — merge theo yêu cầu user trước khi test. Đây là đổi auth breaking: mọi user PHẢI có trong `User_Master` kèm mật khẩu; đã bỏ fallback local → nếu `AUTH_SECRET`/`User_Master` lệch = KHÔNG AI login được. **Việc đầu tiên session sau: smoke test login thật trên GitHub Pages (hard-refresh cache).**
