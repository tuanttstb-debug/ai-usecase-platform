...
**H2 Giai đoạn 3 — Mô hình chấm điểm mới (Đợt 1: lõi member scoring) — CODE XONG 2026-08-25 (branch `feat/h2-scoring`, chưa deploy/merge).**
Thay auto-score 70/30 + SPTD 80-10-10 bằng 2 trục: **Điểm US** (hội đồng 4 teamlead chấm 3 tiêu chí 0–10, 30/40/30, điểm cuối = bình quân) + **Điểm cá nhân** (teamlead chấm 4 tiêu chí 0–10, 30/20/30/20). GAS mới `ScoringServiceH2.gs` (sheets `UC_COUNCIL_SCORE`+`PERSONAL_SCORE`, routes council/personal/h2-leaderboard); FE: review-queue viết lại (hội đồng), trang mới `personal-score.html`, leaderboard 2 tab (Điểm US/Điểm cá nhân), ẩn tab SPTD; module `scoring-h2.js` (unit test 27/27). **KPI PM chốt bản A (30/20/30/20) tại hub AIOS.** Đợt 2: KPI Teamlead 60/40 + PM bản A + khóa học/lan tỏa + trừ milestone + rewrite Playwright 03/04/06. Việc thủ công trước live: deploy GAS (New version) + smoke test → merge main. Chi tiết: `AI_CONTEXT/H2_PLAN.md §8 Giai đoạn 3`. Quản trị chương trình: AIOS hub `binh-dan-hoa-ai-H2/`.
