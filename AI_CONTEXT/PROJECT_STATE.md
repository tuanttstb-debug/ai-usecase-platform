...
**2026-09-21 — CR Kế hoạch T9–T12 ở panel chấm điểm "Hàng đợi Review" + TRUY GỐC "DB không lưu Action Plan" → [TT] đã fix cột (101→105). Thuần FE + tiện ích GAS chạy-tay.**
CR: teamlead khi chấm US ở "Hàng đợi Review" cần thấy đầy đủ "Kế hoạch hành động theo tháng (T9–T12)". Đã thêm mục render vào `uc-detail-view.js` (dùng chung review-queue + dashboard) + `opts.planPlaceholder` để panel chấm điểm LUÔN hiện mục này (ghi "chưa có kế hoạch" khi trống). **Kiểm tra LIVE (gọi route đọc GAS)** phát hiện gốc thật: sheet `MASTER_DATA` **thiếu hẳn 4 cột `Action_Plan_M09..M12`** (102 cột = 106 chuẩn − 4) → Kế hoạch nhập vào bị ghi hụt = "DB không lưu"; panel trống vì không có dữ liệu (code render đúng). Nhân–quả: BUG-1 (18/09) khôi phục header copy từ tab `Data H1` (chưa có 4 cột 12/09) làm rớt cột; `createUseCase_`/`submitWeeklyUpdate_` KHÔNG tự `ensureSheetColumns_(MASTER)` nên cột không quay lại. **Fix:** MỚI `FixMasterHeaders.gs::ensureMasterActionPlanColumns()` (standalone, append 4 cột thiếu ở cuối, idempotent, không đụng data) — **[TT] đã chạy trong Editor → MASTER_DATA 101→105 cột (verify live 4 cột đã có).** Không redeploy Web App. Playwright **127/127**. Cache-bust `?v=20260921b`. Commits `1c97c62` + `b52bf22` ĐÃ PUSH origin/main.
**Còn treo (việc [TT]):** (a) đăng ký/cập nhật 1 US test → xác nhận Kế hoạch lưu & panel hiện; (b) **US cũ trống kế hoạch** (dữ liệu đã rớt lúc nhập) → chủ US nhập lại qua "Cập nhật US"; (c) 🟠 redeploy GAS dồn CR (để `listUseCases_` +4 field action_plan hiện ở render lần đầu có hiệu lực + ExerciseService/SelfScoreService/mapper H1/3 CR từ các phiên trước).
...
**2026-09-14 — RÀ SOÁT UI/UX + ĐỒNG NHẤT các màn CR (US H1 · Bài tập AI · Tự chấm/Duyệt KPI). Thuần FE, KHÔNG redeploy GAS. Playwright 124/124. Cache-bust `?v=20260914`.**
Rà theo concept SHTD + hệ component sẵn có của AIUS → 10 phát hiện đồng nhất, đã sửa (chi tiết `docs/UIUX_AUDIT_20260914.md`). Nổi bật: **(1)** class nút sai `btn--sm`/`btn--ghost` (BEM 2 gạch, không định nghĩa → nút vô hiệu) → quy về canon `.btn-sm`/`.btn-ghost`. **(2)** search view mới về component `.search-box`+`.search-input`+icon (thay `.form-input` trơ). **(3)** modal US H1 về chuẩn `.modal-overlay/.modal-card--wide/.modal-close` (bỏ overlay tự chế). **(4)** badge trạng thái self-score dùng `.badge-*` token (thay hardcode hex → hết hỏng dark mode). **(5)** nút Duyệt/Từ chối dùng `.btn-success`/`.btn-ghost` (bỏ inline bg). **(6)** MỚI `assets/js/ui-confirm.js` (`uiConfirm()` Promise, modal chuẩn) → xóa Bài tập AI có xác nhận. **(7)** MỚI `.list-empty`/`.list-loading`/`.table-wrap` (components.css) dùng chung cho empty/loading/bảng. **(8)** bảng US H1 dùng `.data-table` (giữ hook `.h1-table`). **(9)** nạp **Font Awesome 6** (khớp SHTD) → icon hành động dùng FA thay emoji. **(10)** guard rò rỉ keydown us-h1. **Rule chưng ra lưu hub:** `AIOS/04_Knowledge/references/REF-UIUX-COMPONENT-CONTRACT.md` (6 quy ước + checklist review màn mới). File chạm: `index.html` · MỚI `assets/js/ui-confirm.js` · `assets/css/components.css` · `assets/js/{ai-exercise,self-score,us-h1}.js` · `tests/13-us-h1.spec.js`. **⚠️ Không cần redeploy GAS** (thuần FE); [TT] hard-refresh `?v=20260914`. **Lưu ý:** FA qua CDN cdnjs (không SRI).

---

**2026-09-12 #3 — CR Chấm điểm cá nhân (member tự chấm→teamlead duyệt) + Lan tỏa AI riêng + polish nhãn "Cập nhật US". ⚠️ [TT] REDEPLOY GAS (file mới SelfScoreService.gs).**
Mô hình **Phương án B (KHÔNG đụng engine KPI)**: member ghi sheet staging → teamlead Approve → backend CHỐT sang PERSONAL_SCORE / set Sharing_Achieved (engine `_memberKpiFor_` đọc như cũ → điểm chỉ tính KPI sau Approved). MỚI sheet `PERSONAL_SELF` (M2 4 tiêu chí member tự chấm 0–10 + M3 số khóa thường/trả phí + Evidence_M2/M3 + Status Draft/Submitted/Approved/Rejected) + `SHARING_CLAIM` (lan tỏa: mô tả+Evidence+Status). MỚI `assets/gas-backend/SelfScoreService.gs` (submit/get/pending/review self-score + submit/list/review sharing-claim; auth: chính chủ nộp, teamlead-của-team/admin duyệt; reject bắt buộc lý do; Approved→khóa sửa) + 7 route `self-score-*`/`sharing-claim-*`. FE: MỚI `assets/js/self-score.js` + 2 view — **"Tự chấm KPI"** (member: KPI2 tự chấm 0–10 + KPI3 số khóa + KPI4 khai lan tỏa, xem trạng thái duyệt) + **"Duyệt chấm điểm"** (teamlead/admin: hàng chờ self-score + lan tỏa → Approve/Reject). Quy tắc chốt: review=approve/reject, 1 lần/tháng dương lịch (sửa trước duyệt, khóa sau Approved), lan tỏa **bổ sung song song** auto UC_REUSE≥3, giữ màn teamlead chấm trực tiếp cũ song song, bằng chứng tách theo KPI2/KPI3/KPI4. **Polish:** nhãn "Cập nhật tuần"→"Cập nhật US" (menu/home card/router title/helper); +card home US H1 & Bài tập AI. **Verify:** Playwright **124/124 PASS** (mock). Cache-bust `?v=20260912c`. **⚠️ [TT]:** redeploy GAS — thêm **file mới `SelfScoreService.gs`** + `Config.gs`/`Code.gs` (2 sheet tự tạo khi dùng lần đầu). Plan+quyết định: `docs/PLAN_SCORING_CR_20260912.md`.
> **Ghi chú từ test live (agent):** CR đăng ký tối giản & US H1 PASS trên production. **CR2 "Cập nhật US" danh sách trống = lỗi phía live** (`action=list` trả bản ghi thiếu cột định danh — code repo `listUseCases_` nguyên vẹn, `ensureSheetColumns_` an toàn) → [TT] kiểm header sheet MASTER_DATA + dán lại trọn AdminService.gs khi deploy. CR4 exercise-list thực tế OK (data:[]), dòng đỏ chỉ do GAS cold-start timeout.

---

**2026-09-12 #2 — Tính năng "Bài tập AI" + nâng cấp view US H1 (xem chi tiết + lọc theo trường chung). ⚠️ [TT] REDEPLOY GAS.**
**Bài tập AI:** chia sẻ thao tác AI nhỏ (chưa đủ thành US), KHÔNG tính KPI. Sheet mới `AI_EXERCISE` (`SHEETS.AI_EXERCISE`, headers: Exercise_ID/Title/Description/Prompt/Demo_Link/Owner_*/Team/Created/Updated/Active). GAS `ExerciseService.gs` (list/create/update/delete — xóa mềm Active=FALSE; sửa/xóa = chủ bài hoặc admin) + routes `exercise-list/create/update/delete`. FE `assets/js/ai-exercise.js` + menu "Bài tập AI": 1 trang gộp form đăng (Tiêu đề* + Mô tả + Prompt* + Link demo ổ chung) + tra cứu (tìm kiếm) + xem prompt/copy inline + Sửa/Xóa bài của mình. Nhịp "1 bài/tuần" = khuyến khích (chỉ hiện "tuần này bạn đã đăng X bài", KHÔNG chặn). Demo = link ổ chung (không upload file). **US H1 nâng cấp:** mapper `listUseCasesH1_` trả thêm field nội dung (pain/process/flow/prompt_*/when/usage/business_category); FE `us-h1.js` thêm **modal xem chi tiết** (bấm dòng) + **bộ lọc Team/Workflow/Stage** + mở rộng tìm kiếm (id/tên/team/owner/workflow/stage/status/pain/mô tả). **Verify:** Playwright **120/120 PASS** (mock). **⚠️ [TT]:** redeploy GAS (thêm ExerciseService + routes + mapper H1 mở rộng); sheet `AI_EXERCISE` tự tạo khi đăng bài đầu (ensureSheetColumns_). Hard-refresh `?v=20260912b`.

---

**2026-09-12 — 3 CR: (1) Đăng ký US TỐI GIẢN + (2) "Cập nhật US" thay "Cập nhật tuần" + (3) View "US H1" chỉ đọc. ⚠️ [TT] REDEPLOY GAS + tạo/kiểm sheet `Data H1`.**
Bộ 3 CR liên đới trên spoke `ai-usecase-platform` (FE + GAS). **CR1 — Đăng ký tối giản:** form còn **1 bước** = chọn Workflow → Use Case → 4 ô **Action Plan T9–T12** (bắt buộc ≥1 tháng). Bỏ hẳn painpoint/quy trình/đối tượng/mục tiêu kỳ vọng + toàn bộ bước prompt/demo/hướng dẫn + **Stage** khỏi đăng ký (US mới GAS gán mặc định `S1 - Idea`). Thêm 4 cột `Action_Plan_M09..M12` (cuối HEADERS, self-heal). `REQUIRED_FIELDS_CREATE`: bỏ `Pain_Point/Current_Process/Flow_Description`, thêm `Workflow`. **CR2 — "Cập nhật US" (đổi từ "Cập nhật tuần"):** màn weekly-update thành editor US — Prompt/Luồng AI (accordion mở sẵn = nội dung chính) + Demo + **4 ô Action Plan** + số liệu + % tiến độ; ghi chú KHÔNG còn bắt buộc; **bỏ luồng nâng Stage/checklist** (stageSection giữ trong DOM nhưng luôn ẩn — [[TECH_DEBT]] dead-DOM). GAS `submitWeeklyUpdate_` ghi thêm Action Plan + Demo (nội dung, ghi ngay). **CR3 — View "US H1" (chỉ đọc):** menu mới "US H1" + route GAS `h1-list` đọc sheet `SHEETS.MASTER_H1='Data H1'` (cột giống MASTER_DATA) qua `listUseCasesH1_()`; bảng read-only (Mã·Tên·Team·Owner·Workflow·Stage·Kế hoạch) + tìm kiếm, TÁCH khỏi KPI/leaderboard/chấm điểm H2. **Verify:** Playwright **114/114 PASS** (mock GAS; weekly-update viết lại mock hết flaky + không ghi prod). **⚠️ [TT]:** (a) **redeploy GAS** (Config+UseCaseService+AdminService+Code) — CR1 sẽ LỖI trên GAS cũ (còn require Pain_Point); (b) đảm bảo tab `Data H1` tồn tại (cột giống MASTER_DATA); (c) hard-refresh `?v=20260912a`.

---

**2026-09-11 — Seed WORKFLOW_CATALOG PTKD & QLDM (+33 US / 7 workflow) từ Excel. ⚠️ [TT] chạy `seedWorkflowCatalogPTKD()` trong Editor.**
Chương trình Bình dân hóa AI (AIUS-001) quản trị tại hub AIOS. Nạp US/WF mới PTKD & QLDM vào catalog droplist đăng ký UC từ Excel (sheet "PO - PTKD"). MỚI `assets/gas-backend/WorkflowSeedDataPTKD.gs` (idempotent, dedup Nhom|Workflow|UseCase bất biến khoảng trắng + dryRun). Ánh xạ 3 nhóm canonical khớp `Config.gs TEAM_GROUP_SEED`; **QLDM chia sẻ nhóm "3. Workflow PTKD & QLDM" với PTKD MB/MN** (đã có trong TEAM_GROUP_MAP live). Pre-check read-only 109 dòng live → **+33 US** (PO +0 · chung +2 · PTKD&QLDM +31). **KHÔNG cần redeploy Web App** — chỉ ghi sheet; [TT] dán file vào Editor → dryRun (xác nhận 33) → seed → hard-refresh. Không đụng code FE/GAS route nào khác.

---

**2026-09-09 #3 — Dọn dead code (xóa archive/h1) + TÍNH NĂNG Backup chéo team 2 CHIỀU (sheet `BackupTeam`). ⚠️ [TT] redeploy GAS + chạy `setupBackupTeamSheet()`.**
Sau khi SPA + gỡ duyệt US live (smoke OK): (1) **Dọn** — xóa `archive/h1/` (mô hình H1 nghỉ, git giữ history) + gỡ dead code duyệt US sót (`renderRejectedCard`/`REJECTED_PREVIEW`/`_rejectedList`). (2) **Backup chéo team 2 CHIỀU** (anh: Tutv3/CV2 ↔ lead CV1 backup hộ NHAU) — chốt **sheet `BackupTeam` RIÊNG trên SPREADSHEET AIUS** (KHÔNG chèn cột vào User_Master của SHTD; đúng chuẩn dự án `SHEETS.BACKUP_TEAM`+`getOrCreateSheet_`). *(Lần đầu em lỡ đặt trên spreadsheet SHTD qua openById — anh nhắc, đã sửa về spreadsheet AIUS + dời hàm từ AuthTokenService→AdminService cho đúng miền.)* **Schema pair: `Teamlead_A|Team_A|Teamlead_B|Team_B|Active|Note|Updated_At` — 1 DÒNG = 1 cặp backup 2 chiều** (A chấm được Team_B ∧ B chấm được Team_A, khi Active=TRUE). GAS `AdminService`: MỚI `getBackupTeamsFor_(username)` (đọc sheet AIUS, 2 chiều) + `setupBackupTeamSheet()` (chuẩn: `getOrCreateSheet_` + header tím/freeze + dòng ví dụ Active=FALSE); `AuthTokenService.authLogin_` trả `backup_teams` (gọi global cross-file). GAS `isChampionForTeam_` (gate submit `ScoringServiceH2:399`): team mình OR ∈ getBackupTeamsFor_. FE `auth.js` (persist session) + `personal-score.js` (filter + nhãn "backup"). **Verify:** FE — teamlead CV2 backup CV1 thấy member CV1(nhãn backup)+CV2, KHÔNG CV3; test 09 4/4; syntax GAS 3 file OK; 0 orphan. **⚠️ [TT]:** redeploy GAS (Config+AdminService+AuthTokenService) → chạy **`setupBackupTeamSheet()`** 1 lần (tạo sheet **BackupTeam trên spreadsheet AIUS**) → điền dòng `tutv3|CV2|<lead_CV1>|CV1|TRUE|...` → 2 người **đăng nhập lại**. Sheet vắng = không ai backup (backward-compat).
