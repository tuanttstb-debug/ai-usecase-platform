...
Sau khi SPA + gỡ duyệt US live (smoke OK): (1) **Dọn** — xóa `archive/h1/` (mô hình H1 nghỉ, git giữ history) + gỡ dead code duyệt US sót (`renderRejectedCard`/`REJECTED_PREVIEW`/`_rejectedList`). (2) **Backup chéo team 2 CHIỀU** (anh: Tutv3/CV2 ↔ lead CV1 backup hộ NHAU) — chốt **sheet `BackupTeam` RIÊNG** (không chèn cột vào User_Master; anh OK đặt trên **spreadsheet SHTD** = master quản lý). **Schema pair: `Teamlead_A|Team_A|Teamlead_B|Team_B|Active|Note|Updated_At` — 1 DÒNG = 1 cặp backup 2 chiều** (A chấm được Team_B ∧ B chấm được Team_A, khi Active=TRUE). GAS `AuthTokenService`: MỚI `getBackupTeamsFor_(username)` (đọc sheet, 2 chiều) + `setupBackupTeamSheet()` (tạo sheet chuẩn: header tím + freeze + dòng ví dụ Active=FALSE) + `authLogin_` trả `backup_teams`. GAS `isChampionForTeam_` (gate submit `ScoringServiceH2:399`): team mình OR ∈ getBackupTeamsFor_. FE `auth.js` (persist session) + `personal-score.js` (filter + nhãn "backup"). **Verify:** FE — teamlead CV2 backup CV1 thấy member CV1(nhãn backup)+CV2, KHÔNG CV3; test 09 4/4 pass; syntax FE+GAS OK; sạch ref cột cũ. **⚠️ [TT]:** redeploy GAS (AuthTokenService+AdminService) → chạy **`setupBackupTeamSheet()`** 1 lần trong GAS Editor (tạo sheet trên spreadsheet SHTD) → điền 1 dòng `tutv3|CV2|<lead_CV1>|CV1|TRUE|...` → 2 người **đăng nhập lại**. Sheet vắng = không ai backup (an toàn, backward-compat).
