...
**H2 in progress (branch `feat/h2-shared-auth`, chưa merge main):** Giai đoạn 1 — **Auth dùng chung user/mật khẩu với SHTD-Dashboard** hoàn tất code + test local. Login đổi từ username-only → **username+password** (SHA-256 + token HMAC), nguồn user = sheet `User_Master` trên spreadsheet SHTD `1cpg1p…`; role `champion`→`teamlead`; quản lý user + đổi/đặt-lại mật khẩu qua User_Master; `change-password.html` mới. GAS đã deploy (AUTH_SECRET, URL không đổi). Test: Playwright 98/98 · SPTD 34/34 · KPI 38/38 · ID 14/14. Kế hoạch đầy đủ: `AI_CONTEXT/H2_PLAN.md`. **Chờ smoke test login thật + merge main.**
