...
- [ ] **[P0 — VIỆC ĐẦU TIÊN Giai đoạn 2] Deploy GAS + seed + smoke test** (GAS Editor AI US): (1) Edit deployment → New version; (2) chạy **`seedWorkflowCatalog()`** (import 69 US + seed Team→Nhóm + self-heal cột MASTER); (3) smoke test đăng ký (Workflow lọc theo Team → US dependent → "Khác") + trang `workflow-catalog.html`; (4) merge `feat/h2-workflow-input` → `main`.
